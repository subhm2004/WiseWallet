...
**A full-stack personal finance platform** with microservices architecture, AI-powered insights, Splitwise-style expense sharing, and production-grade security.

Built with **Next.js 15**, **Express microservices**, **Neon PostgreSQL**, **Prisma**, **Groq AI**, **Inngest**, **Brevo**, and **ArcJet**.
...
10. [Where to Get API Keys](#where-to-get-api-keys)
11. [Database](#database)
12. [API Gateway & Routing](#api-gateway--routing)
13. [API Reference](#api-reference)
14. [Frontend Routes & Pages](#frontend-routes--pages)
15. [Authentication & Sessions](#authentication--sessions)
16. [Split Expenses (Splitwise-style)](#split-expenses-splitwise-style)
17. [Background Jobs (Inngest)](#background-jobs-inngest)
18. [Email System](#email-system)
19. [Security (ArcJet, JWT, Rate Limits)](#security-arcjet-jwt-rate-limits)
20. [PWA & Offline](#pwa--offline)
21. [Available Scripts](#available-scripts)
22. [URLs When Running](#urls-when-running)
23. [Testing Features](#testing-features)
24. [Troubleshooting](#troubleshooting)
25. [Production Deployment Notes](#production-deployment-notes)
...
WiseWallet is a **monorepo** personal finance application that helps users:

- Track **bank accounts** (Current & Savings) and **transactions**
- Set **monthly budgets**, **category budgets**, and **savings goals**
- Scan **receipts with AI** (Groq) to auto-fill transaction details
- Get **AI finance coaching** and monthly insights
- Manage **recurring bills / subscriptions**
- **Split expenses** with friends via shareable invite links (no bank account required for splits)
- Receive **email alerts** (budget warnings, bill reminders, weekly digest, monthly reports)
- Customize the **dashboard** with toggleable widgets
...
└─────────────────────────────────┬───────────────────────────────────────┘
                                  │
                                  │  /api/* rewritten by Next.js
                                  ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                         API Gateway (:8080)                              │
│              CORS · ArcJet Shield · Bot Detection · Proxy               │
└───┬─────────┬─────────────┬──────────────────┬────────────┬────────────┘
    │         │             │                  │            │
    ▼         ▼             ▼                  ▼            ▼
 /api/auth  /api/accounts  /api/transactions  /api/budgets  /api/notifications
    │         │             │  /api/splits     │            │
    ▼         ▼             ▼                  ▼            ▼
...
```
1. User submits form on /transaction/create
2. frontend/lib/api.js → POST /api/transactions
3. Next.js rewrite → http://localhost:8080/api/transactions
4. Gateway proxies → http://localhost:4003/ (transaction-service)
5. JWT verified → ArcJet rate limit check → Prisma create
6. Account balance updated in same DB transaction
7. JSON response → toast success → redirect to dashboard
```
...
- [Groq](https://console.groq.com) — AI receipt scan & coach (free tier)
- [Brevo](https://brevo.com) — Email sending (300/day free)
- [ArcJet](https://app.arcjet.com) — Security & rate limits (free tier)
...
Inngest worker function
  → POST http://localhost:4005/send
  → Header: x-internal-key: INTERNAL_SERVICE_SECRET
  → notification-service calls Brevo API
...
Conflicting public file and page route for `/icon.svg`. Cosmetic — does not affect app functionality.
...
For extended setup instructions, see [`setup.md`](./setup.md).
